...
export type VisionDetailMode = ReplyDetailMode;
...
export interface StreamMeta extends WebSearchInfo {
  kodoHit: boolean;
  semanticHit: boolean;
  sentImage: boolean;
...
export interface StreamDonePayload extends WebSearchInfo {
  reply: string;
...
export function getStoredSessionId(username: string): string | null {
  return localStorage.getItem(`${SESSION_KEY_PREFIX}${username}`);
}

export function setStoredSessionId(username: string, sessionId: string): void {
  localStorage.setItem(`${SESSION_KEY_PREFIX}${username}`, sessionId);
}

export function clearStoredSessionId(username: string): void {
  localStorage.removeItem(`${SESSION_KEY_PREFIX}${username}`);
}

export async function getOrCreateSessionApi(sessionId?: string | null): Promise<{
  sessionId: string;
  ...
  if (!res.ok) throw new Error(data.error || '会话初始化失败');
  return data;
}
...
export async function fetchSessionMessages(sessionId: string): Promise<ChatMessage[]> {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.error || '读取会话失败');
...
export async function deleteSessionApi(sessionId: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    throw new Error(data.error || '清空会话失败');
  }
}
    ...
    for (const block of blocks) {
      ...
      try {
        ...
        if (event === 'meta') callbacks.onMeta?.(parsed);
          ...
          callbacks.onDone?.(parsed.reply ?? '', parsed.usage, parsed.toolsUsed, {
            webSearchSummary: parsed.webSearchSummary,
            webSearchProvider: parsed.webSearchProvider,
            webSearchFailed: parsed.webSearchFailed,
          });
...
export async function checkHealth(): Promise<{
  ...
  ttsModel?: string;
  ttsVoice?: string;
  mcpToolDetails?: Array<{ name: string; description?: string }>;
}> {
